...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  private XMLConfigBuilder(XPathParser parser, String environment, Properties props) {
    // 创建Configuration对象，并通过TypeAliasRegistry注册一些Mybatis内部相关的类的别名
  ...
  private void environmentsElement(XNode context) throws Exception {
    if (context != null) {
      if (environment == null) {
        /**
         * 方法参数没有传递environment，则解析sqlMapConfig.xml中的
      ...
      for (XNode child : context.getChildren()) {
        ...
        // 判断id和environment的值是否相等，default是否相等
        if (isSpecifiedEnvironment(id)) {
          ...
          // 将environment对象存储到Configuration中
   ...
   *   <property name="Driver" value="${dataSoruce.driver}"/>
   *   ...
   *   </dataSource>
  ...
  private DataSourceFactory dataSourceElement(XNode context) throws Exception {
    if (context != null) {
      // 判断数据源类型 POOLED UNPOOLED JNDI
      String type = context.getStringAttribute("type");
      Properties props = context.getChildrenAsProperties();
      // 根据dataSource的type找到具体的工厂
